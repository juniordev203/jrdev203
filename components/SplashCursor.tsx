...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: { r: number; g: number; b: number }) {
      ...
      gl.uniform1f(splatProgram.uniforms.radius, correctRadius(config.SPLAT_RADIUS / 100.0) || 0);
      blit(velocity.write);
      velocity.swap();
    ...
    function updatePointerMoveData(pointer: PointerPrototype, posX: number, posY: number, color: { r: number; g: number; b: number }) {
      ...
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
      pointer.deltaX = correctDeltaX(pointer.texcoordX - pointer.prevTexcoordX) || 0;
      pointer.deltaY = correctDeltaY(pointer.texcoordY - pointer.prevTexcoordY) || 0;
